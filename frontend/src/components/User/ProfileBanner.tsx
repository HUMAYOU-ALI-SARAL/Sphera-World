...
import AchievTwo from "@/public/img/achievements/logo-2.png";
import AchievThree from "@/public/img/achievements/logo-3.png";
import AchievFour from "@/public/img/achievements/logo-4.png";
...
const AccountInfo = ({ accountId }: { accountId: string }) => {
  ...
  const handleCopyAccountId = () => {
    navigator.clipboard
      ...
      .catch((error) => {
        console.error("Failed to copy AccountID:", error);
      });
  };
    ...
    <div className={`relative w-full ${styles.profileBanner}`}>
      <div className="relative">
        <Image
          alt="banner"
          src={
            userProfile?.bgImgUrl
              ? process.env.NEXT_PUBLIC_BACKEND_URL + userProfile.bgImgUrl
              : DefaultBg.src
          }
          width={0}
          height={0}
            ...
            <div className="flex items-start justify-between w-full px-10 flex-wrap">
              <div className="flex flex-col">
                <div className="flex items-start">
                  <span className="text-24 font-semibold pt-2">{`${userProfile?.firstName || ""
